//
//  UILabel+TPEAttributeTextTapAction.m
...
#import "UILabel+TPEAttributeTextTapAction.h"
#import <objc/runtime.h>
#import <CoreText/CoreText.h>
#import <Foundation/Foundation.h>

@interface TPEAttributeModel : NSObject
...
@implementation TPEAttributeModel

@end

@implementation UILabel (TPEAttributeTextTapAction)
...
- (id<TPEAttributeTapActionDelegate>)delegate
{
    return objc_getAssociatedObject(self, _cmd);
}

- (void)setDelegate:(id<TPEAttributeTapActionDelegate>)delegate
{
...
- (void)tpe_addAttributeTapActionWithStrings:(NSArray <NSString *> *)strings tapClicked:(void (^) (UILabel * label, NSString *string, NSRange range, NSInteger index))tapClick
{
    [self tpe_getRangesWithStrings:strings];
...
- (void)tpe_addAttributeTapActionWithRanges:(NSArray<NSString *> *)ranges tapClicked:(void (^)(UILabel *, NSString *, NSRange, NSInteger))tapClick
{
    [self tpe_getRangesWithRanges:ranges];
    self.userInteractionEnabled = YES;
...
- (void)tpe_addAttributeTapActionWithRanges:(NSArray<NSString *> *)ranges delegate:(id<TPEAttributeTapActionDelegate>)delegate
{
    [self tpe_getRangesWithRanges:ranges];
    self.userInteractionEnabled = YES;
...
- (void)touchesBegan:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event
{
    ...
    BOOL ret = [self tpe_getTapFrameWithTouchPoint:point result:^(NSString *string, NSRange range, NSInteger index) {
        
        if (weakSelf.isTapEffect) {
            
            [weakSelf tpe_saveEffectDicWithRange:range];
            
            [weakSelf tpe_tapEffectWithStatus:YES];
        }
...
- (void)touchesEnded:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event
{
    ...
    if (self.isTapEffect) {
        [self performSelectorOnMainThread:@selector(tpe_tapEffectWithStatus:) withObject:nil waitUntilDone:NO];
    }
    ...
    BOOL ret = [self tpe_getTapFrameWithTouchPoint:point result:^(NSString *string, NSRange range, NSInteger index) {
        ...
        if (weakSelf.delegate && [weakSelf.delegate respondsToSelector:@selector(tpe_tapAttributeInLabel:string:range:index:)]) {
            [weakSelf.delegate tpe_tapAttributeInLabel:weakSelf string:string range:range index:index];
        }
    }];
    if (!ret) {
        [super touchesEnded:touches withEvent:event];
...
- (BOOL)tpe_getTapFrameWithTouchPoint:(CGPoint)point result:(void (^) (NSString *string , NSRange range , NSInteger index))resultBlock
{
    ...
    CGFloat total_height =  [self tpe_textSizeWithAttributedString:self.attributedText width:self.bounds.size.width numberOfLines:0].height;
    ...
    CGAffineTransform transform = [self tpe_transformForCoreText];
    ...
    for (CFIndex i = 0; i < count; i++) {
        ...
        CGRect flippedRect = [self tpe_getLineBounds:line point:linePoint];
        
        CGRect rect = CGRectApplyAffineTransform(flippedRect, transform);
        
        CGFloat lineOutSpace = (self.bounds.size.height - total_height) / 2;
        
        rect.origin.y = lineOutSpace + [self tpe_getLineOrign:line];
        ...
        if (CGRectContainsPoint(rect, point)) {
            ...
            for (int j = 0; j < link_count; j++) {
                ...
                TPEAttributeModel *model = self.attributeStrings[j];
...
- (CGAffineTransform)tpe_transformForCoreText
{
    return CGAffineTransformScale(CGAffineTransformMakeTranslation(0, self.bounds.size.height), 1.f, -1.f);
}

- (CGRect)tpe_getLineBounds:(CTLineRef)line point:(CGPoint)point
{
    CGFloat ascent = 0.0f;
    ...
    height = [self tpe_textSizeWithAttributedString:attributedString width:self.bounds.size.width numberOfLines:0].height;
    return CGRectMake(point.x, point.y , width, height);
}

- (CGFloat)tpe_getLineOrign:(CTLineRef)line
{
    CFRange range = CTLineGetStringRange(line);
    if (range.location == 0) {
        return 0.;
        ...
        return [self tpe_textSizeWithAttributedString:attributedString width:self.bounds.size.width numberOfLines:0].height;
    }
}

- (CGSize)tpe_textSizeWithAttributedString:(NSAttributedString *)attributedString width:(float)width numberOfLines:(NSInteger)numberOfLines
{
    @autoreleasepool {
...
- (void)tpe_tapEffectWithStatus:(BOOL)status
{
    if (self.isTapEffect) {
...
- (void)tpe_saveEffectDicWithRange:(NSRange)range
{
...
- (void)tpe_getRangesWithStrings:(NSArray <NSString *>  *)strings
{
    ...
    [strings enumerateObjectsUsingBlock:^(NSString * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        if (range.length != 0) {
            ...
            totalStr = [totalStr stringByReplacingCharactersInRange:range withString:[weakSelf tpe_getStringWithRange:range]];
            
            TPEAttributeModel *model = [TPEAttributeModel new];
            model.range = range;
            model.str = obj;
...
- (void)tpe_getRangesWithRanges:(NSArray <NSString *>  *)ranges
{
    if (self.attributedText == nil) {
        self.isTapAction = NO;
        return;
...
- (NSString *)tpe_getStringWithRange:(NSRange)range
{
